...
import { database, blockchain, Entity } from 'flair-sdk';
...
export const getOrCreatePool = async function (chainId: number, address: string, cache = true) {
  return multiplex(`pool:${chainId}:${address}`, 60000, async () => {
    ...
    if (pool && pool.token0Address && pool.token1Address) {
      return pool;
    }
